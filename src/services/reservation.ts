...
export const reservationService = {
   ...
   */
  transition: async (queueId: number, action: ReservationTransitionAction) => {
  const response = await api.patch<ReservationTransitionResponse>(`/api/queue/${queueId}/${action}`)
    return response.data
  },
